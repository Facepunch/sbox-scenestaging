...
public class ParticleFloatControlWidget : ControlWidget
{
	...
	public ParticleFloatControlWidget( SerializedProperty property ) : base( property )
	{
		...
		{
			if ( p != type ) return;
			RebuildForType( p.GetValue<ParticleFloat.ValueType>() );
		};

		RebuildForType( type.GetValue<ParticleFloat.ValueType>() );

	}

	void RebuildForType( ParticleFloat.ValueType type )
	{
		...
		{
			var controlA = ControlWidget.Create( Target.GetProperty( "CurveA" ) );
			ControlArea.Add( controlA );
		}
	}
