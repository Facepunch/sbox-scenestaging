...
internal readonly record struct TimeSelection( MovieTimeRange PeakTimeRange, TimeSelection.Fade FadeIn, TimeSelection.Fade FadeOut )
{
	...
	{
		if ( time < PeakTimeRange.Start )
		{
			return FadeIn.Interpolation.Apply( (float)FadeInTimeRange.GetFraction( time ) );
		}

		if ( time > PeakTimeRange.End )
		{
			return FadeOut.Interpolation.Apply( 1f - (float)FadeOutTimeRange.GetFraction( time ) );
		}
