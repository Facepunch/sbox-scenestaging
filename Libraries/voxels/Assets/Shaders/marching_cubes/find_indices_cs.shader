...
CS
{
    ...
    void MainCs(uint3 dispatchId : SV_DispatchThreadID)
    {
        ...
        for (int i = 0; i < entry.TriangleCount; i++)
        {
            ...
            IndexBuffer[baseIndex + i * 3 + 0] = GetVertexIndex(index, tri.Vertices[0]);
            IndexBuffer[baseIndex + i * 3 + 1] = GetVertexIndex(index, tri.Vertices[1]);
            IndexBuffer[baseIndex + i * 3 + 2] = GetVertexIndex(index, tri.Vertices[2]);
        }
    }
}
